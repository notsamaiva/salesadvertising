...
const PaymentForm = ({ handlePaymentConfirmation, billingInfo, totalAmount }) => {
  ...
  const response = await fetch('http://localhost/pricing_app/payment.php', {
    ...
    body: JSON.stringify({
      ...
      amount: Math.round(totalAmount * 100), // Montant en cents
      billing_info: billingInfo,
      return_url: 'http://localhost:3000/confirmation',
    }),
  });

  // Vérifiez si la réponse est de type JSON
